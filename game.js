...
class Game {
        ...
        for (let i = this.bullets.length - 1; i >= 0; i--) {
            ...
            if (!this.isMultiplayer) {
                for (let j = this.enemies.length - 1; j >= 0; j--) {
                    ...
                    if (hit) {
                        console.log('[HIT] enemy health:', enemy.health, 'damage:', bullet.damage);
    ...
    updateUI() {
        ...
        const expFill = document.getElementById('exp-fill');
        if (expFill) expFill.style.width = (this.player.exp / this.player.expToLevel * 100) + '%';
        
        const dayNightDisplay = document.getElementById('day-night-display');
        if (dayNightDisplay) {
            dayNightDisplay.textContent = this.isNight ? __('night') : __('day');
        }
        
        const autoDisplay = document.getElementById('auto-display');
        if (autoDisplay) {
            autoDisplay.textContent = this.autoMode ? __('autoMode') : __('manual');
    ...
    drawParticles() {
        for (const p of this.particles) {
            ...
            this.ctx.globalAlpha = Math.max(0, lifeRatio);
            this.ctx.fillStyle = p.color;
            this.ctx.beginPath();
            this.ctx.arc(p.x, p.y, p.radius * easeOut, 0, Math.PI * 2);
            this.ctx.fill();
        }
        this.ctx.globalAlpha = 1;
    }
