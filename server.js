...
function updateBullets(roomId) {
    ...
    for (let i = gs.bullets.length - 1; i >= 0; i--) {
        ...
        bullet.distance += Math.hypot(bullet.x - prevX, bullet.y - prevY);
        
        if (bullet.x < -100 || bullet.x > MAP_WIDTH + 100 || 
            bullet.y < -100 || bullet.y > MAP_HEIGHT + 100 ||
            bullet.distance > bullet.maxDistance) {
            gs.bullets.splice(i, 1);
            continue;
        }
        
        const steps = Math.max(1, Math.ceil(Math.hypot(bullet.x - prevX, bullet.y - prevY) / 5));
        let hit = false;
        ...
        for (let step = 0; step <= steps; step++) {
            ...
            for (const enemy of gs.enemies) {
                ...
                if (dist < bullet.radius + enemy.radius) {
                    ...
                    enemy.hitFlash = 3;
                    console.log(`[updateBullets] HIT enemy id=${enemy.id} hp=${Math.round(enemy.health)}`);
                    ...
                    if (enemy.health <= 0) {
                        ...
                        const shooter = room.get(bullet.ownerId);
                        if (shooter) {
                            shooter.kills = (shooter.kills || 0) + 1;
                            sendPlayersList(roomId);
                        }
                        const eidx = gs.enemies.indexOf(enemy);
                        if (eidx !== -1) gs.enemies.splice(eidx, 1);
                    }
                    hit = true;
                    break;
                }
...
function simulateBulletTrajectory(roomId, startX, startY, dirX, dirY, maxDist, damage, radius, shooterId, gs, room) {
    const stepSize = Math.min(5, radius + 5);
    const steps = Math.max(1, Math.ceil(maxDist / stepSize));
    
    for (let i = 0; i <= steps; i++) {
        const cx = startX + dirX * i * stepSize;
        const cy = startY + dirY * i * stepSize;
        
        if (cx < -200 || cx > MAP_WIDTH + 200 || cy < -200 || cy > MAP_HEIGHT + 200) {
            break;
        }
        ...
        if (shooterId) {
            for (const [playerId, player] of room) {
                ...
                if (dist < radius + player.radius) {
                    ...
                    if (player.health <= 0) {
                        const shooter = room.get(shooterId);
                        if (shooter) {
                            shooter.kills = (shooter.kills || 0) + 1;
                            sendPlayersList(roomId);
                            broadcastToRoom(roomId, {
...
function checkBulletHit(roomId, bx, by, bvx, bvy, damage, radius, shooterId) {
    const gs = gameStates.get(roomId);
    const room = rooms.get(roomId);
    if (!gs || !room) return { hit: false };
    
    console.log(`[checkBulletHit] room=${roomId} enemies=${gs.enemies.length} bx=${Math.round(bx)} by=${Math.round(by)} damage=${damage} radius=${radius}`);
    gs.enemies.forEach((e, i) => {
        console.log(`[checkBulletHit] enemy[${i}] id=${e.id} x=${Math.round(e.x)} y=${Math.round(e.y)} hp=${Math.round(e.health)} radius=${e.radius}`);
    });
    
    const bulletSpeed = Math.hypot(bvx, bvy);
    const dirX = bulletSpeed > 0 ? bvx / bulletSpeed : 0;
    const dirY = bulletSpeed > 0 ? bvy / bulletSpeed : 0;
    
    const result = simulateBulletTrajectory(
        roomId, bx, by, dirX, dirY, 3000, damage, radius, shooterId, gs, room
    );
    
    if (result.hit) {
        console.log(`[checkBulletHit] HIT ${result.enemy ? 'enemy' : 'player'} id=${result.enemy ? result.enemy.id : result.player ? result.player.id : '?'}`);
    } else {
        console.log(`[checkBulletHit] NO hit found`);
    }
...
function broadcastGameState(roomId) {
    ...
    const state = {
        type: 'gameState',
        enemies: gs.enemies.filter(e => e.health > 0).map(e => ({
            id: e.id,
            x: Math.round(e.x),
            y: Math.round(e.y),
            radius: e.radius,
...
function handleShoot(ws, message) {
    ...
    // 1. IMMEDIATE collision check (traces 800px in the bullet's direction)
    // This covers close and medium range shots without waiting for the next tick
    ...
    if (result.hit && result.enemy) {
        ...
        if (result.enemy.health <= 0) {
            const room = rooms.get(ws.roomId);
            if (room) {
                spawnLoot(ws.roomId, result.enemy.x, result.enemy.y, 'exp', 25);
                if (Math.random() < 0.3) {
                    const lootType = LOOT_TYPES[Math.floor(Math.random() * LOOT_TYPES.length)];
                    spawnLoot(ws.roomId,
                        result.enemy.x + (Math.random() - 0.5) * 30,
                        result.enemy.y + (Math.random() - 0.5) * 30,
                        lootType.type,
                        lootType.effect
                    );
                }
                const shooter = room.get(ws.userId);
                if (shooter) {
                    shooter.kills = (shooter.kills || 0) + 1;
                    sendPlayersList(ws.roomId);
                }
            }
            const eidx = gs.enemies.indexOf(result.enemy);
            if (eidx !== -1) {
                console.log(`[handleShoot] Removing dead enemy id=${result.enemy.id}`);
                gs.enemies.splice(eidx, 1);
            }
        }
    } else if (!result.hit) {
        // 2. No immediate hit - add to persistent tracking for long-range shots
        gs.bullets.push({
            id: 'b_' + (gs.bulletIdCounter++),
            x: message.x,
            y: message.y,
            vx: message.vx,
            vy: message.vy,
            radius: message.bulletRadius || 5,
...
function handleJoinOfficialRoom(ws, message) {
    ...
    for (let i = 1; i <= OFFICIAL_ROOM_COUNT; i++) {
        ...
        if (count < MAX_PLAYERS_PER_ROOM && count >= maxPlayers) {
            maxPlayers = count;
            bestRoom = roomId;
        }
    }
